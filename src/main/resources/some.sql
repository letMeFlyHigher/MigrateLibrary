...
select * from TAB_OMIN_CM_CC_AREA;
...
WHERE substr(TAB_OMIN_META_NETWORK.NETWORKPK,0,32) = TAB_OMIN_CM_CC_STATION.C_STATION_ID(+) and
      TAB_OMIN_META_REGSTATION.REGSTATIONID(+) = TAB_OMIN_CM_CC_STATION.C_STATION_ID and
     -- SUBSTR(TAB_OMIN_META_OBSVELMT.OBSVELMTPK(+),0,36) = TAB_OMIN_META_NETWORK.NETWORKPKAND
     TAB_OMIN_META_NETWORK.NETWORKTYPE='01';


select * from TAB_OMIN_META_OBSVELMT O
WHERE O.OBSVELMTNAME = '自记降水';

select *  from TAB_OMIN_META_OBSVELMT o
where o.OBSVELMTNAME = '大型蒸发量' and o.SWITCHDATE is null;

select o.SWITCHDATE from tab_omin_meta_network n,tab_omin_meta_obsvelmt o
where substr(o.OBSVELMTPK,0,36) = n.NETWORKPK and n.networktype = '01' and o.OBSVELMTNAME = '大型蒸发量' and o.SWITCHDATE is not null;
